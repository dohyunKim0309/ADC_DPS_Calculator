...
|---|---|---|---|---|---|
| **Cog'Maw** | 325 ✓ | 40 ✓ | 8.75 ✓ | 0.7 ✓ | W 40 ✓ · E [40,55,70,85,100] ✓ · R 40+40/스택(≤9, 40–400) ✓ · **Q [src?]** |
...
- H-MANA-2: 평타 무비용·OOM 시 캐스트 스킵(자원 부족이면 미시전).
...
## 4.1 확정 수치 (출처: LoL Wiki + DDragon 16.13.1 교차검증)
...
| `ad_growth` | **3.1** | **[user?]** Wiki=3.1, DDragon raw=0 — **Ezreal과 동일한 DDragon 데이터 결함**(H-EZ-1 선례) → Wiki 채택. 확인 요청 |
...
- **Q 부식침(Caustic Spittle)**: 패시브 공속 `[5,10,15,20,25]%` · 액티브 마법 `[80,125,170,215,260] + 0.9·AP` ·
  **방어력+마저 −`[16,20,24,28,32]%`, 4초** · 쿨 7s · 마나 [src?].
- **W 바이오아케인(Bio-Arcane Barrage)**: 온힛 마법 = 대상 **최대체력의 `[3,3.75,4.5,5.25,6]%` + (100AP당 1.5%)** ·
  몹 캡 100(더미=챔피언이라 미적용) · 지속 8s · 마나 40.
- **E 공허진흙(Void Ooze)**: 마법 `[70,110,150,190,230] + 0.65·AP` · 쿨 12s · 마나 [40,55,70,85,100] (슬로우=DPS무관).
- **R 리빙아틸러리(Living Artillery)**: 마법(일반) `[100,140,180] + 0.75·추가AD + [35,40,45]%·AP`,
  **대상 HP ≤ 40%면 ×2** · 쿨 `[2,1.5,1]s` · 마나 **40 + 40/스택(≤9스택, 40–400), 스택 8s** · 발사후 0.6s.
- 패시브 Icathian Surprise(사망 폭발 true) = **DPS 무관, 미모델**.

## 4.2 클래스 모델링 (`CogMaw(Champion)`) — 각 가설 §4.4
- **W(시그니처)** → `get_champion_onhit(target)` 반환 `(0, w_magic)`,
  `w_magic = (w_pct[idx] + 0.00015·total_ap) · target.max_hp` (w_pct=[.03,.0375,.045,.0525,.06]).
  - **[H] W 전투 내내 항시 유지**(유나라 Q 항시활성 관례). W 마나(40)는 [H] 유지비용 미부과
    (항시활성 가정과 일관) — 단 Cog'Maw는 W 단발 장시간이라 영향 적음. (대안: 8s마다 W 재시전
    이산화 — 원하면 §6 확정.)
...
- **Q 액티브 / E / R** → 이벤트 캐스트 시스템(Kai'Sa·Corki·Ezreal 미러), 쿨마다 자동 시전 + **마나 게이트(§3.3)**.
  ...
  - R: `추가AD = total_ad − base_attack_ad`(현재레벨 기본AD). 대상 HP≤40% 분기 ×2.
...
- `simulate_cogmaw_core_path(full_path, core_tier, doran_key, boots_key, rune_as_bonus)`:
  레지스트리로 아이템 생성, 룬(§4.3 컨트롤), skill_plan(W 항시활성→캐스트 불필요; Q/E/R 오토 쿨마다),
...
## 4.4 Phase 1 가설
- H-KOG-1: ad_growth=3.1(DDragon 결함, Wiki 채택 — Ezreal 선례).
- H-KOG-2: W 항시활성·`(pct + 0.00015·AP)·maxHP` 온힛·구인수 2배·증폭/Shadowflame 적용·W 유지비 미부과.
- H-KOG-3: Q 패시브 공속 상수(시전 해제 무시).
- H-KOG-4: Q 액티브 방/마저 %감소(감소→관통 순서)·E·R 쿨마다 시전.
- H-KOG-5: R 추가AD=total_ad−base_attack_ad, HP≤40% ×2, 마나램프 스로틀.
...
## 6. 스펙 리뷰 — 확인 요청 항목
1. **Cog'Maw `ad_growth=3.1`** 확정?(DDragon=0은 Ezreal과 동일 결함 → Wiki 채택)
2. **버프형 Q 리팩터 깊이**(§3.4): *활성=마나-게이트, 버프효과 보존*(추천) vs *이벤트 캐스트 완전이전*.
3. **W 유지비/항시활성**(§4.2): 항시활성·유지비 미부과(추천) vs 8s 재시전 이산화.

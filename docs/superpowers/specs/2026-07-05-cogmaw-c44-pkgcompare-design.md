...
### 5. C44 증폭 채널 확장 — 평타 패키지 전체 (사용자 정정, 2026-07-06)
- 사용자 확인: "C44는 코그모 W 마법 온힛에 적용되는 것이 맞다" — 기존 엔진의 '평타 물리 기본딜만'
  적용은 과소 모델. [H-C44-ONHIT-1, 사용자 인게임 지식]
- 결정(채널 질문 무응답 → 권장안 채택): **평타에 실리는 패키지 전체** —
  `phys_base·magic_base·phys_onhit·magic_onhit`에 별도 배수 `(1+c44)` 적용, 그리고
  `_last_damage_amp`에도 곱해 **은화살 true 온힛도 증폭**(H-VAYNE-W-2의 LDR 관례와 일관).
  스킬 직격딜(Q/E/R 등)은 미적용 — 툴팁 "기본 공격 시". 별도 배수(가산 damage_multiplier 와
  곱연산 분리) 시맨틱은 유지.
- 파급: c44 포함 빌드 전 챔피언 상향(Ashe/Jinx top1 yuntal-c44-ldr-ie, CogMaw c44 빌드 등 —
  의도된 정정). 회귀 baseline 5빌드는 c44 미포함 → 무영향. CogMaw 재실행으로 CTRL2 수치 갱신.
- 테스트: 마법 온힛 ×1.10 검증(가짜 온힛 아이템), `_last_damage_amp`에 c44 반영 검증,
  기존 range 테스트 4종 불변.

### 6. C44 증폭 채널 재정정 — W 전용 버그 모델 (사용자 2차 정정, 2026-07-06)
- 사용자 확인: C44 증폭이 코그모 W 온힛에 적용되는 것은 **인게임 일시적 버그**이며, 룬 온힛·
  아이템 온힛 등 일반 온힛에는 적용되지 않는 것이 맞다. → §5(전 채널 확장, H-C44-ONHIT-1)는
  과대 모델로 **폐기**.
- 올바른 모델: ① `Champion.get_one_hit_damage`의 C44 블록을 **원래(평타 물리 기본딜만)로 원복**
  (`magic_base`/온힛 채널/`_last_damage_amp` 곱 제거 — 은화살 true도 미증폭으로 복귀).
  ② `CogMaw.get_champion_onhit`에서 **W 온힛만** `(1+c44 modifier)` 곱 — 버그 태그
  `[H-C44-KOGW-BUG-1]`, "라이엇 픽스 시 이 블록 제거" 명시.
- 테스트: 일반 마법 온힛 미증폭(100 유지), `_last_damage_amp`==1.0 복귀, 코그모 W 온힛만
  ×1.10(get_champion_onhit 직접 비교, c44 유/무), 기존 range 4종 불변. (§5의 테스트 2종 대체)
- 파급: c44 일반 온힛빌드 상승분 롤백, 코그모 W 증폭은 유지 → 재실행으로 수치 재갱신.

...
스탯/가격의 단일 출처는 data/items_data.py 이며, 레지스트리가 인스턴스에 주입한다.
여기 남은 생성자 리터럴은 레거시 참고값으로, 런타임 수치에 영향을 주지 않는다.
"""
...
class UmbralGlaive(Item):
    """그림자 검 (Umbral Glaive) — AD 60 / 물리 관통력 18 / 스킬가속 15. [H-UMBRAL-1]

    밤의 추적자: 1초 동안 적에게 보이지 않으면 챔피언 대상 다음 기본 공격에
    `50 + 물리 관통력 150%` 고정(true) 피해를 추가로 입힌다(1회 소비).
    비노출 판정은 챔피언의 `unseen_since`(비노출 시작 시각) — 베인 R 중 Q(구르기)만
    이 상태를 만들며, 다른 챔피언은 None 이라 순수 스탯 아이템으로 동작한다.
    암전(와드 감지)은 전투 DPS와 무관해 미모델.
    (나무위키 그림자 검 / 스탯·가격 출처는 items_data.ITEMS['umbral'])
    """

    NIGHTSTALKER_UNSEEN_SECONDS = 1.0
    NIGHTSTALKER_BASE_DAMAGE = 50.0
    NIGHTSTALKER_LETHALITY_RATIO = 1.5

    def __init__(self):
        super().__init__("Umbral Glaive", ad=60, lethality=18, cdr=15)
        self.cost = 2800
        # 이미 소비한 비노출 구간의 시작 시각. 같은 구간에서 두 번 터지지 않게 한다.
        self.nightstalker_consumed_from = None

    def on_hit(self, target, champion):
        unseen_since = getattr(champion, "unseen_since", None)
        if unseen_since is None:
            return 0, 0, 0, 0
        elapsed = getattr(champion, "_combat_time", 0.0) - unseen_since
        if elapsed < self.NIGHTSTALKER_UNSEEN_SECONDS:
            return 0, 0, 0, 0
        if self.nightstalker_consumed_from == unseen_since:
            return 0, 0, 0, 0  # 같은 비노출 구간에서는 1회만

        self.nightstalker_consumed_from = unseen_since
        damage = (self.NIGHTSTALKER_BASE_DAMAGE
                  + self.NIGHTSTALKER_LETHALITY_RATIO * champion.lethality)
        return 0, 0, 0, damage  # 고정(true) 온힛
...
class WitsEnd(Item):
    ...
    """
    def __init__(self):
        super().__init__("Wit's End", as_percent=0.50, mr=45, tenacity=0.2)
        self.cost = 2800
        self.fray_magic = 45

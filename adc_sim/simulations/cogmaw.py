...
ITEM_SHORT = {
    ...
    "pd": "PD", "ie": "IE", "yuntal": "Yun", "ldr": "LDR", "rabadon": "Rabadon",
...
class SimCache:
    """아이템 집합을 키로 코그모 DPS·골드를 메모이즈한다."""

    def __init__(self, package, keystone_cls):
        """시작 패키지와 키스톤을 고정한 코그모 탐색 캐시를 초기화한다."""
        self.kw = {
    ...
    def sim(self, items_tuple):
        ...
        self.misses += 1
        result = simulate_cogmaw_core_path(list(items_tuple), len(items_tuple), **self.kw)
...
def main_legacy_ranking():
    """교체 전 코그모 4코어 전수 랭킹·패키지 비교·그래프를 실행한다."""
...
def run_cli(args=None):
    """기본 receding-horizon 또는 `legacy-ranking` 호환 모드로 코그모 CLI를 실행한다."""
    import sys

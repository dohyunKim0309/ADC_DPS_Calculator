...
# ═══════════════════════════════════════════════════════════════════════════════
# 하프 후보 열거는 2026-08-31 유나라 구현을 그대로 들여온 것이다(그때는 yunara 에서 import).
# 유나라 쪽은 2026-09-15 에 "아이템별 예산창 [ceil100(가격/2), +100]"(data/recipe_states.py)로
# 갈아탔지만, 카이사까지 바꾸면 진화 판정 타이밍이 같이 움직여 별도 검증이 필요하다.
# 그래서 카이사는 옛 전역 캡 1600 규칙을 **로컬 복사본으로 유지**한다 — 이관은 후속 작업.
KAISA_HALF_TIER_GOLD_CAP = 1600
FALLBACK_RECIPES = {
...
def _half_component_subsets(next_key, cap=KAISA_HALF_TIER_GOLD_CAP):

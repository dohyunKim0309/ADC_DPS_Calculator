"""베인 LT(치속) vs PtA(집공) 비교 — 룬별 표 2개.
각 표: 그 룬 RelDPG 상위 top_n + 레퍼런스(컨트롤). 컬럼: 코어 1~4 별 DPS·DPG + RelDPG.
...
def _fmt_build(r, col_build):
    p = r["path"]
    label = f"{'-'.join(ITEM_SHORT.get(k, k) for k in p)} [{r['pkg_label']}]"
...
if __name__ == "__main__":
    ...
    print(f"\n=== Vayne LT vs PtA — 룬별 표 2개 · 각 Top {top_n} + Reference (≈1분) ===")
    run(top_n=top_n)

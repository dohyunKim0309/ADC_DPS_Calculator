...
- **스킬 시전 시간(auto-attack lockout)** [H-YUNARA-CAST-1, 유나라 파일럿]: 베이스 `Champion`에 `cast_lockout_until`(기본 0=락아웃 없음, `init_combat_state`에서 리셋) — **이 시각까지 평타 불가**. `engine.run_simulation`이 평타 게이트에서 `current_time < cast_lockout_until`이면 다음 평타를 락아웃 종료까지 지연(미설정 챔피언은 0이라 **기존 동작 불변**). 유나라 W(스킬샷)는 시전 시 `cast_lockout_until = time + w_cast_time()` 설정. `w_cast_time()=max(0.225, 0.45/(1+추가공속))`(LoL Wiki: 추가공속 비례 0.45→0.225). **캔슬 가능**이라 시전(락아웃) 종료 후 다음 평타는 0.33 클립 유지(`w_cancelable`; False면 클립 없이 full interval). **Q(정신 수양)·R(초월)=즉발 자가버프 → 시전 0**(기존 0.33 클립만), E=이동기 미모델. **핵심 발견: 유나라 평타 간격(공속캡 3.0 바닥 0.333s, 실전 ~0.46s)이 W 시전 시간(≤0.45s)보다 항상 커서 W 는 평타 손실 없이 위빙 → DPS 영향 ≈ 0**(캔슬-위빙 모델). 락아웃 기구는 시전>간격일 때만 평타를 지연(테스트 `tests/test_yunara_cast_time.py`). 시전 시간을 타 챔피언으로 확장하려면 해당 스킬 시전 지점에서 `cast_lockout_until` 설정.

...
- **유나라 템트리 리포트(HTML)**: `python -m tools.yunara_report_data` (곡선 데이터 재생성, 시뮬 ≈2분)
  → `python -m tools.build_explorer_report` (템플릿+JSON 합쳐 `docs/reports/yunara_report.html`).
...
docs/reports/ ─ 코어 결정 탐색기 리포트: **챔피언 중립 템플릿**(explorer.template.html) +
  ...
  챔피언 이름·문구는 데이터의 `copy` 블록에만 둔다(README.md 참조)
...
- **상대 타깃 아키타입** [사용자 확정 2026-09-16] — `simulations/target_archetypes.py`:
  **딜러 / 브루저(기본) / 탱커** 3종. 완성 코어와 하프 구간이 같은 표를 읽으므로
  ...
  · 딜러 = **원딜 8종 기본 스탯 평균**(체력 608.1+102.75 / 방어 25.0+4.39 / 마저 33.0+1.1,
    ...
    → **1280/50/39 → 1995/89/49**(4~5코어 체력은 평균에서 −100). ⚠️ 딜러만 타깃 레벨이 한 칸 낮다(5코어에 18레벨을 못 찍는
...
- **결론 템트리** [2026-09-21 재스윕 — 아키타입 3 × 시나리오 4, 1:1:1 혼합, mDPG 1차]:
  ...
  | | 적1 | 적2 | 적3 | 혼합 |
  |---|---|---|---|---|
  | 딜러 | 크라켄-구인수-경계-유령무희-C44 | 크라켄-루난-경계-구인수-폭풍갈퀴 | 크라켄-루난-경계-C44-유령무희 | 크라켄-루난-경계-구인수-C44 |
  | 브루저 | 구인수-크라켄-경계-유령무희-무한 | 크라켄-구인수-루난-도미닉-내셔 | 크라켄-루난-구인수-경계-그림자불꽃 | 크라켄-루난-구인수-경계-그림자불꽃 |
  | 탱커 | 구인수-경계-크라켄-몰왕검-C44 | 크라켄-루난-구인수-도미닉-몰왕검 | 크라켄-루난-구인수-경계-그림자불꽃 | 크라켄-루난-구인수-도미닉-몰왕검 |

  12상황 등장 횟수: 크라켄 12 · 구인수 11 · 경계 9 · 루난 9 · C44 4 · 유령무희 3 · 도미닉 3 ·
  그림자불꽃 3 · 몰왕검 3 · 폭풍갈퀴 1 · 무한 1 · 내셔 1.
  ⚠️ **mDPG 1차로 바꾸면서 윤탈·무한이 사실상 사라졌다**(윤탈 0회, 무한 1회). 구조적 이유가
  있다 — 윤탈은 **구매 코어 치확 0% 가정** 탓에 자기 칸 mDPG 가 낮고 보상이 다음 칸에 나온다
  (브루저·혼합 1코어: 윤탈 mDPG 67.8 / score 510.0 vs 크라켄 83.2 / 516.0 — score 는 3.5 차인데
  mDPG 는 15.4 차). 스택·크리 시너지 아이템(윤탈·무한·C44)이 같은 방식으로 저평가되므로
  **"실전 윤탈 시작과 어긋나는 결과"는 모델 한계로 읽을 것**. 완화안(미실행): 윤탈 칸의 mDPG 를
  "구매 칸 + 다음 칸" 합으로 보거나, 구매 코어 치확 가정을 12% 같은 중간값으로 올린다.
  직전 기준(score 1차 · γ=0.7)에서는 혼합 3종 모두 `크라켄-윤탈-루난-도미닉-무한` 계열이었다.
- **AP 풀**: score 1차 시절에는 전 시나리오에서 한 번도 안 뽑혔으나, mDPG 1차로 바꾼 뒤
  **5코어 그림자불꽃 3회·내셔 1회**가 들어온다(값싼 마지막 칸 효율). 1코어 DPG로
...
### Kai'Sa (`champion.py` KaiSa + `simulations/kaisa.py`)
- **타깃 = 아키타입 모듈**(사용자 확정 2026-09-21). 옛 고정표(1700/50/30 … 3000/150/95)를 버리고
  `target_archetypes` 의 딜러/브루저/탱커를 쓴다. `set_target_archetype(name)` 이 완성 코어와
  하프 구간(`_kaisa_half_tier_target`)을 함께 바꾼다 — 유나라와 같은 규약.
  ⚠️ 이 교체로 **KaiSa 절대 DPS 가 ±3~7% 이동**했고 `tests/_baseline_dps.json` 을 재캡처했다.
  옛 수치를 인용한 문서·주석이 있으면 그쪽이 낡은 것이다.
- **탐색기**(`tools/kaisa_explorer_data.py`): 기존 하프 코어 RH(`solve_greedy_half_kaisa` ·
  `_score_combo_half` · `kaisa_sim_half`) 점수식 그대로, 순열 전수 대신 집합 단위 DP.
  `--verify` 로 원본과 1코어 후보 score 가 Δ0 인지 확인한다. 유나라와 다른 점:
  **5코어 하프도 잰다** · **적 수 축 없음**(단일 대상 시뮬, 시나리오 `solo` 하나) ·
  슬롯별 후보 풀이 좁고 **앞 두 아이템 합계 AD 75·공속 65% 하한**이 추가로 걸린다 ·
  윤탈 키는 `yuntal`(유나라는 `yuntal25`) · γ 는 프로젝트 기본 0.8.
- **막다른 길 제외 규칙**: 2코어 스탯 하한 탓에 1코어 내셔·C44 는 다음 칸 후보가 0 이 된다.
  원본 `_enumerate_future_combos` 도 그런 경로를 안 만들므로 탐색기도 후보에서 뺀다
  (`_next_exists`). 안 빼면 score 가 미래 항 없이 잘려 순위가 왜곡된다.
- **결론**(2026-09-21, mDPG 1차 · 패키지 A(도란검+광전사+핏빛길) · 치속+체력차극복 · 파편 AS10%+AD5.4):
  딜러 `무한-유령무희-크라켄-구인수-도미닉` / 브루저 `구인수-윤탈-도미닉-크라켄-무한` /
  탱커 `구인수-윤탈-내셔-그림자불꽃-라바돈`. **유나라와 달리 윤탈이 2코어로 살아남는다**
  (온힛·진화 스케일이라 구인수-윤탈 연계가 그 칸에서 바로 값을 낸다). 딜러 1코어는 무한이
  mDPG 92.8 로 1위지만 score 는 775 로 윤탈 1079·구인수 1071 에 크게 밀린다.
...
- **Q 평캔/재적립 규칙(Ashe·Yunara 공통)**: Q 만료 후 평타로 **전용 스택**을 재적립해야 재사용(Ashe `q_stacks`≥4, Yunara `q_stacks`≥8=4평타; 활성 시 0 리셋). Ashe Q는 전역 `hit_count`가 아닌 전용 스택 사용(과거 즉시 재활성 버그 수정). **Ashe** 는 스킬 평캔 직후 다음 평타 간격을 **`ANIM_CANCEL_CLIP`(=0.33s, `champion.py` 모듈 상수)** 로 상한 클리핑(평타캔슬 가정). **유나라는 이 클립을 쓰지 않음**(스킬 전부 비캔슬 — 시전 시간 항목 참조).
- **유나라 W**(`adc_sim/champion.py`)는 엔진 스킬 이벤트로 모델링: Q활성(초월)이면 강화 W(파멸의 궤적, 궁 레벨 색인 160/320/480 +1.2추가AD +0.75AP, 쿨5s), 비활성이면 기본 W(심판의 궤적, W 레벨 색인, 적중+6초 DoT, 쿨10s). **`[Hypothesis]` 나무위키 수치(CDragon API 미검증)**. W 레벨/궁 레벨은 `CORE_YUNARA_LEVELS`의 `w_level`/`r_level`(Q선마→W차선마 가정). `Yunara(w_enabled=False)`로 W 이전 동작 복귀(검증/AB용).
- **스킬 시전 시간(cast time)** [H-YUNARA-CAST-1, 유나라 파일럿]: 베이스 `Champion`에 두 채널(기본 0, `init_combat_state`에서 리셋; 미설정 챔피언은 **기존 동작 불변**). `engine.run_simulation`이 평타 게이트에서 소비 — **캔슬 가능/불가 구분**(사용자 확정):
  - **캔슬 가능(흡수형) `cast_lockout_until`**: 이 시각까지 평타 불가하되 평타 타이머는 밑에서 계속 흐름 → 시전 시간이 평타 간격 안에 들면 **무손실(위빙)**. 시전 후 회복 캔슬로 0.33 클립.

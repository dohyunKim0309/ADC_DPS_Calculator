...
- **스킬 시전 시간(cast time)** [H-YUNARA-CAST-1, 유나라 파일럿]: 베이스 `Champion`에 두 채널(기본 0, `init_combat_state`에서 리셋; 미설정 챔피언은 **기존 동작 불변**). `engine.run_simulation`이 평타 게이트에서 소비 — **캔슬 가능/불가 구분**(사용자 확정):
  ...
  - **유나라 W(심판/파멸의 궤적, 스킬샷)=평타캔슬 불가** → 가산형. 시전 시 `cast_delay_pending += w_cast_time()`, `w_cast_time()=max(0.225, 0.45/(1+추가공속))`(LoL Wiki: 추가공속 비례 0.45→0.225). `w_cancelable=False`(True로 두면 흡수형+0.33클립 경로). **이전의 W 무조건 0.33 클립(캔슬 가정)은 제거** — W는 비캔슬이라 시전이 순수 딜로스. Yunara DPS 약 −7%(kraken-pd-ie-ldr T1~4, 회귀 기준선 갱신). **Q(정신 수양)·R(초월)=즉발 자가버프 → 시전 0**(기존 0.33 클립만), E=이동기 미모델. 테스트 `tests/test_yunara_cast_time.py`. 타 챔피언 확장은 해당 스킬 시전 지점에서 두 채널 중 하나 설정.

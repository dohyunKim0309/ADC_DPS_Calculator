...
from adc_sim.simulations.yunara import SimCache, SHARD_SCENARIOS
...
# 하프 티어 선택은 시뮬 본체와 같은 공통 엔진(receding.select_half)을 탄다 —
# 리포트가 탐색과 다른 하프 구성을 그리지 않도록 스펙을 그대로 빌려 쓴다.
SPEC = Y.build_spec()


def apply_archetype(arch):
    """yunara 모듈의 활성 타깃 아키타입 교체 — 완성 코어와 하프(보간) 둘 다 바뀐다.

    ⚠️ `Y.CORE_TARGET_STATS` 만 갈아끼우면 안 된다 — 하프 구간은 `ACTIVE_ARCHETYPE` 를
    읽으므로 표만 바꾸면 완성 코어만 움직이고 하프는 기본 아키타입에 남는다.
    """
    Y.set_target_archetype(arch["id"])
        ...
        for sname, weights in SCEN:
            ...
            for tier in range(1, len(b["path"]) + 1):
                ...
                if tier < 5:   # 5코어 하프는 프로젝트 기본대로 생략
                    hd, hg = blend(weights, lambda c, tc, d=done, n=nxt:
                                   receding.select_half(SPEC, c, d, n)[:2])
                    comps = receding.select_half(SPEC, caches[1], done, nxt)[2]
